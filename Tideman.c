#include <cs50.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>

// Max number of candidates
#define MAX 9

// preferences[i][j] is number of voters who prefer i over j
int preferences[MAX][MAX];


// locked[i][j] means i is locked in over j
bool locked[MAX][MAX];

// Each pair has a winner, loser
typedef struct
{
    int winner;
    int loser;
}
pair;

// Array of candidates
string candidates[MAX];
pair pairs[MAX * (MAX - 1) / 2];

int pair_count;
int candidate_count;

// Function prototypes
bool vote(int rank, string name, int ranks[]);
void record_preferences(int ranks[]);
void add_pairs(void);
void sort_pairs(void);
void lock_pairs(void);
void print_winner(void);

int main(int argc, string argv[])
{
    // Check for invalid usage
    if (argc < 2)
    {
        printf("Usage: tideman [candidate ...]\n");
        return 1;
    }

    // Populate array of candidates
    candidate_count = argc - 1;
    if (candidate_count > MAX)
    {
        printf("Maximum number of candidates is %i\n", MAX);
        return 2;
    }
    for (int i = 0; i < candidate_count; i++)
    {
        candidates[i] = argv[i + 1];
    }

    // Clear graph of locked in pairs
    for (int i = 0; i < candidate_count; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
            locked[i][j] = false;
        }
    }

    pair_count = 0;
    int voter_count = get_int("Number of voters: ");

     
    for (int u=0; u<candidate_count; u++){
        for (int h=0; h<candidate_count; h++){
            preferences[u][h]=0;
        }
    }
    // Query for votes
    for (int i = 0; i < voter_count; i++)
    {
        // ranks[i] is voter's ith preference
        int ranks[candidate_count];
        // Query for each rank
        for (int j = 0; j < candidate_count; j++)
        {
            
            string name = get_string("Rank %i: ", j + 1);
            vote(j, name, ranks);
            if (!vote(j, name, ranks))
            {
                printf("Invalid vote.\n");
                return 3;
            }
        }   
        record_preferences(ranks);
        printf("\n");
    }
    add_pairs();
    sort_pairs();
    lock_pairs();
    print_winner();
    return 0;
}

int index(string name)
{
     for(int i=0; i<candidate_count; i++){
        if (!strcmp(name, candidates[i])){ 
     
            return i; 
     
        }
    }
    
    return -1;
}

bool vote(int rank, string name, int ranks[])
{    
    string voter_rank[candidate_count];
    int check_name = index(name);
    voter_rank[rank] = name;
    if (check_name != -1){
    
       ranks[rank] = check_name;
       return true;
       
   }
    
    return false;
    
}

int ranks_index(int ranks[], int i)
{
    for(int b=i; b<candidate_count; b++){
        
        for(int j=0; j<candidate_count; j++){
    
            if(b == ranks[j]){
    
                return j;
            }
        }
        
    }
    
    return -9;
}

// Update preferences given one voter's ranks
void record_preferences(int ranks[])
{
    string name = NULL;
    for (int k=0; k<candidate_count; k++){
       
            for (int f=0; f<candidate_count; f++){
                
                if(k == ranks[f]){
                    
                    for (int j=0; j<candidate_count; j++){
                        int r_index = ranks_index(ranks, j);
                       
                        if (f<r_index){
                            preferences[k][j] += 1;
                           
                        }
                        
                        /*printf("%d ", pref[k][j]);
                        if (j==candidate_count-1){
                            printf("\n");
                        }*/
                    }              
                }
           }
    }
    return;
}

// Record pairs of candidates where one is preferred over the other
void add_pairs(void)
{
    int count=0;
    
    for (int i=0; i<candidate_count; i++){
        int j=count;
        
        while(j<candidate_count){
            
            if ((preferences[i][j] > preferences[j][i]) && (i!=j)){
                pairs[i].winner = i;
                pairs[i].loser=j;
                pair_count++;

            }
        
            else if ((preferences[i][j] < preferences[j][i]) && (i!=j))
            {
            pairs[i].winner = j;
            pairs[i].loser=i;
            pair_count++;
            }
            j++;
        }
        count++;
        
    }

    return;
}

int compare(const void * elem1, const void * elem2){
    pair f = *((pair*)elem1);
    pair s = *((pair*)elem2);
    int margin1 = preferences[f.winner][f.loser] - preferences[f.loser][f.winner];
    int margin2 = preferences[s.winner][s.loser] - preferences[s.loser][s.winner];
    return margin2-margin1;
}
// Sort pairs in decreasing order by strength of victory
void sort_pairs(void)
{

qsort(pairs, pair_count, sizeof(pair), compare);
  
}

bool is_loop (int index, bool check[])
{
    if(check[index])
        return true;
    check[index]=true;
    for (int i=0; i<candidate_count; i++){
        if (locked[index][i] && is_loop(i, check))
            return true;
        }
        return false;
    }


bool loop(int start_index)
{
    bool check[candidate_count];
    for (int i=0; i<candidate_count; i++)
        check[i]=false;
    return is_loop(start_index, check);
    
}

// Lock pairs into the candidate graph in order, without creating cycles
void lock_pairs(void)
{

for (int i=0; i<pair_count; i++){
    locked[pairs[i].winner][pairs[i].loser] = true;
    if (loop(pairs[i].winner))
    locked[pairs[i].winner][pairs[i].loser] = false;
}

    return;
}
bool is_source(int index)
{
    for (int i=0; i<candidate_count;i++)
    if (locked[i][index])
        return false;
    return true;
}

int source()
{
    for (int i=0; i<candidate_count; i++){
        if (is_source(i))
            return i;
    }
        return -1;
    
    
}
// Print the winner of the election
void print_winner(void)
{
int index=source();
if (index != -1){
    printf("%s\n", candidates[index]);
}
return;
}

